...
and manageable. It employs two key concepts `Resolve` and `Branch` much like the
essential steps in building a house, laying the foundation and then expanding
upon it to add the final touches.
...
const fetchUserDataBranch = morphism(options)({
  f: (c) => {
    ...
    return await fetch(`https://api.example.com/users/${userId}`).then((res) =>
      res.json()
    );
  },
});
