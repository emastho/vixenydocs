<script>
	...
	function hexToRgb() {
		const hexColor = getComputedStyle(document.documentElement).getPropertyValue('--ashes').trim();
...
<div id="canvascontainer" bind:this={canvascontainer}>
	<canvas id="buffer" bind:this={bufferCanvas} />
	<canvas id="canvas" bind:this={mainCanvas} />
</div>
